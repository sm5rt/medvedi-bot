...
async def save_users(data: Dict[str, Any]):
    for uid, user_data in data.items():
        await users_col.replace_one({"_id": uid}, user_data, upsert=True)

async def load_club_history() -> list:
    history = []
    async for entry in club_history_col.find({}):
        # Удаляем _id, если не нужен
        entry.pop("_id", None)
        history.append(entry)
    return history

async def save_club_history(history: list):
    await club_history_col.delete_many({})
    if history:
        await club_history_col.insert_many(history)

async def load_season_config():
    doc = await season_config_col.find_one({"_id": "season"})
    if not doc:
...
async def save_season_config(start: datetime, end: datetime):
    await season_config_col.replace_one(
        {"_id": "season"},
...
def season_time_left():
    _, SEASON_END = load_season_config_sync()
...
# Синхронная версия для использования вне async (например, в фоновых задачах — но лучше избегать)
# Лучше переписать фоновые задачи на async (см. ниже)
...
async def check_club_changes(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Загружаем последнее состояние клуба
    last_doc = await last_state_col.find_one({"_id": "last_tags"})
    ...
    await last_state_col.replace_one(
        {"_id": "last_tags"},
...
def get_season_dates_sync():
    """Синхронная загрузка для функций вне async — НЕ ИСПОЛЬЗОВАТЬ в основном коде!"""
    import asyncio
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    start, end = loop.run_until_complete(load_season_config())
    return start, end

def season_time_left():
    _, SEASON_END = get_season_dates_sync()
    now = datetime.now(timezone.utc)
    delta = SEASON_END - now
    if delta.total_seconds() < 0:
        return 0, 0
...
# ================== АДМИН-КОМАНДЫ ==================
# (остальные команды: version, reload_season, broadcast, setnorm, deleteuser, admincmds, history_cmd)
# Здесь для краткости опущены — они аналогичны, но с async/await и MongoDB.
# Если нужно — могу добавить полную версию всех админ-команд.
...
def main():
    ...
    # Добавьте остальные: version, reload, broadcast и т.д.
